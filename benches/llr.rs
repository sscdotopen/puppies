...
#[inline(always)]
pub fn llr_cse_pre(k11: u64, k12: u64, k21: u64, k22: u64, logs: &Vec<f64>) -> f64 {

  let xlx_all = x_log_x(k11 + k12 + k21 + k22);

  let log_k11 = logs[k11 as usize];
  let log_k12 = logs[k12 as usize];
  let log_k21 = logs[k21 as usize];
  let log_k11_12 = logs[(k11 + k12) as usize];
  let log_k11_21 = logs[(k11 + k21) as usize];
...
fn bench_llr_cse_pre(bench: &mut Bencher) {
  ...
  let logs = pre_compute_logarithms((MAX_COOCCURRENCES + MAX_OBSERVATIONS) as usize);

  bench.iter(|| {
    for _ in 0..NUM_REPETITIONS {
      bencher::black_box(llr_cse_pre(rng.gen_range(0, MAX_COOCCURRENCES),
...
fn bench_llr_cse(bench: &mut Bencher) {
  ...
  bench.iter(|| {
    for _ in 0..NUM_REPETITIONS {
      bencher::black_box(llr_cse(rng.gen_range(0, MAX_COOCCURRENCES),
                                 rng.gen_range(0, MAX_OBSERVATIONS),
                                 rng.gen_range(0, MAX_OBSERVATIONS),
                                 rng.next_u64()));
    }
...
fn bench_llr_no_cse(bench: &mut Bencher) {
  ...
  bench.iter(|| {
    for _ in 0..NUM_REPETITIONS {
      bencher::black_box(llr_no_cse(rng.gen_range(0, MAX_COOCCURRENCES),
                                    rng.gen_range(0, MAX_OBSERVATIONS),
                                    rng.gen_range(0, MAX_OBSERVATIONS),
                                    rng.next_u64()));
    }
