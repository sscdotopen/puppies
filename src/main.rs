...
fn main() {
    ...
    let batch_size: usize = std::env::args().nth(5).unwrap().parse().unwrap();
    let k: usize = std::env::args().nth(6).unwrap().parse().unwrap();

    puppies::incremental_indicators(file, num_users, num_items, pool_size, batch_size, k);
}